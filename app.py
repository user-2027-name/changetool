...
# --- 1. 計算用の関数 (内部計算・Excel変換用) ---
def time_str_to_timedelta(time_str):
    """'13:30' を Excelが認識できる形式(timedelta)に変換する"""
...
# --- 2. 共通のデータ変換関数 ---
def transform_data(df):import streamlit as st
import pandas as pd
import re
import requests
from io import BytesIO, StringIO
...
if processed_df is not None:
    ...
    output = BytesIO()
    try:
        # Excel書き出し用のデータを作成（Excel用に時間を変換）
        export_df = processed_df.copy()
        time_cols = ["始業時刻", "終業時刻", "運転時間", "休憩時間", "拘束時間合計", "実働時間", "休息時間"]
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            ...
            # Excel上での表示形式を [h]:mm に設定（24時間を超えても正しく表示される形式）
            time_format = workbook.add_format({'num_format': '[h]:mm'})
            
            # 時間列にフォーマットを適用
            for i, col in enumerate(export_df.columns):
                if col in time_cols:
                    worksheet.set_column(i, i, None, time_format)

        st.download_button(
            label="📥 計算可能なExcelをダウンロード",
            data=output.getvalue(),
            file_name=f"拘束時間管理表_{date.today()}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
